...
description: Choose compatibility-aware scope for runtime, API, configuration, client-wrapper, and guardrail behavior changes in openai-guardrails-js. Use before initial implementation and each review-feedback batch to decide whether to patch, reset the design, preserve compatibility, or reject unsupported cases.
---
...
2. Determine the latest release tag to use as the compatibility baseline from `origin` first, and only fall back to local tags when remote tags are unavailable:
   ...
   echo "$BASE_TAG"
   ```
   If fetching tags is unavailable, use the newest local `v*` tag and report that baseline as potentially stale.
...
- Call out changes to supported released behavior or durable formats in the plan and handoff. Do not change `package.json` or `pyproject.toml` versions unless the task explicitly includes a release or version update.
...
- Treat released exports from `src/index.ts`, emitted declaration shapes, configuration schemas, guardrail result objects, exception behavior, and CLI arguments as compatibility-sensitive.
- Preserve drop-in compatibility for `GuardrailsOpenAI` and `GuardrailsAzureOpenAI` across both Chat Completions and Responses resources. Forward request parameters and request options without silently dropping caller intent.
- Preserve omitted OpenAI SDK values when `undefined` is meaningful. Do not normalize an omitted request or client option to a concrete library default unless the upstream contract requires it.
- Keep `pre_flight`, `input`, and `output` stage ordering, tripwire behavior, `suppressTripwire`, and `raiseGuardrailErrors` coherent across streaming, non-streaming, direct client, and Agents SDK paths.
- Treat JSON configuration, file-loaded configuration, eval datasets, caller history, and provider responses as untrusted inputs. Validate before OpenAI requests, guardrail execution side effects, file writes, or externally visible mutation where practical.
- Preserve conversation ordering, tool-call identity, response data, request options, and guardrail results across conversion boundaries. Fail clearly instead of silently dropping a supported value.
- Keep registry lookup, Zod validation, configuration loading, resource forwarding, conversation normalization, and masking in their existing owning pipelines instead of duplicating them in individual checks or wrappers.
- Retry or replay only when it is safe to assume the OpenAI request or guardrail side effect was not accepted. Streaming and eval paths can duplicate external work after ambiguous failure.
- Verify OpenAI or Azure SDK behavior against authoritative documentation and installed types. A live probe requires explicit approval and the repository credential policy.
