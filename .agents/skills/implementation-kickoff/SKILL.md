...
- Use read-only GitHub access when remote PR evidence is required. Never run `gh` on this machine.
...
Record the original requirement, success criteria, intended target (`origin/main` unless the user states otherwise), task-owned paths, compatibility boundary, intentionally unsupported cases, and required repository skills. Maintain a working plan when the task is multi-step. A plan, review packet, ledger, trace, or temporary report is operational-only by default; do not add it to the shipped-path manifest unless the original requirement explicitly makes that exact path a committed deliverable.
...
Place every `review_state.py --complete-diff-output` artifact outside the repository and worktree. A repository-local output path is invalid because writing it would mutate or replace reviewed content after fingerprinting.

Run the repository's applicable completion gates against the complete task-owned diff on the final-base candidate. Before freezing the first review fingerprint, inspect the actual final commit-hook configuration and run the exact safe, non-committing equivalent of every hook step that can rewrite a shipped path. Repeat rewriting steps until content-idempotent, and verify generated-file hashes or provenance after normalization. Generate final-review evidence with `review_state.py --complete-diff-output <complete.diff>` so ordinary task-owned untracked files are present in the reviewed diff without staging them. For runtime code, tests, examples, build or test behavior, or behavior-impacting docs, run `$implementation-final-review` and follow its frozen-fingerprint review and final-gate orchestration. Run `$code-change-verification` for every eligible runtime, test, example, or build/test change. Honor every skill's fingerprint, ordering, and invalidation rules. This repository has no Changesets workflow; do not add release metadata or change package versions unless the task explicitly requires it.
...
Invoke `$pr-draft-summary` only after review and every applicable final repository gate apply to the final content. Give it a self-contained packet containing the original requirement, implementation scope contract, important decisions and intent, complete changed-path inventory including untracked files, final diff and statistics, compatibility notes, issue references, and takeover provenance.
...
The handoff validator accepts required coauthors only from the commit's actual Git trailer block and pins the conventional `:` separator independently of ambient Git configuration. A `Co-authored-by`-shaped line in the message body does not satisfy attribution.

Run `PYTHONDONTWRITEBYTECODE=1 python3 .agents/skills/implementation-kickoff/scripts/validate_handoff.py --repo <worktree> --base <final-base> --expected-branch <branch> --shipped-path-manifest <manifest>`. The shipped-path manifest is mandatory; validation cannot succeed without comparing the committed path set to it. It must be a finite regular file read from one opened descriptor. Its exact repository-relative shipped paths are NUL-terminated filesystem-byte entries, and it excludes operational artifacts. For a takeover, also pass `--required-trailer-email <verified-email>` for each identity that must be credited. For example, create a two-path manifest with `printf '%s\0' 'src/a.ts' 'src/b.ts' > task.paths`.
